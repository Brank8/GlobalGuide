...
const SearchDestination = ({
  ...
  const searchDestination = async () => {
    ...
    const url = `${BASE_URL}?days=${days}&destination=${encodeURIComponent(
      destination
    )}`;
    const options = {
      method: "GET",
    ...
    try {
      ...
      const urlWeather = `http://api.weatherapi.com/v1/forecast.json?key=${API_KEY_WEATHER}&q=${encodeURIComponent(
        destination
      )}&days=7`;
